...
/* Rotation per Kconfig schaltbar: bei Landscape (CONFIG_CUSTOM_VIEW_ROTATE_CANVAS=n) no-op */
static inline void rotate_canvas_maybe(lv_obj_t *canvas, lv_color_t *cbuf) {
#if IS_ENABLED(CONFIG_CUSTOM_VIEW_ROTATE_CANVAS)
...
static void draw_top(lv_obj_t *widget, lv_color_t cbuf[], const struct status_state *state) {
    ...
    /* Output (USB/BLE) */
#if IS_ENABLED(CONFIG_CUSTOM_VIEW_SHOW_OUTPUT)
    {
...
#endif /* CONFIG_CUSTOM_VIEW_SHOW_OUTPUT */

    /* WPM (Zahl + optional Sparkline) */
  ...
  #if IS_ENABLED(CONFIG_CUSTOM_VIEW_SHOW_WPM_SPARKLINE)
    int max = 0;
    int min = 255;
    for (int i = 0; i < 10; i++) {
  ...
  #endif /* CONFIG_CUSTOM_VIEW_SHOW_WPM_SPARKLINE */
#endif /* CONFIG_CUSTOM_VIEW_SHOW_WPM */

    /* Rotation (optional) */
    rotate_canvas_maybe(canvas, cbuf);
}

static void draw_middle(lv_obj_t *widget, lv_color_t cbuf[], const struct status_state *state) {
#if !IS_ENABLED(CONFIG_CUSTOM_VIEW_SHOW_PROFILES)
    /* Feature aus: nichts zeichnen */
    return;
#endif
        ...
        if (state->profiles_connected[i]) {
            ...
            for (int j = 0; j < segments; ++j) {
                lv_canvas_draw_arc(canvas,
                                   circle_offsets[i][0], circle_offsets[i][1],
                                   13,
                                   360.0f / segments * j + gap / 2.0f,
                                   360.0f / segments * (j + 1) - gap / 2.0f,
                                   &arc_dsc);
            }
        }
...
ZMK_SUBSCRIPTION(widget_battery_status, zmk_battery_state_changed);
  ...
  #endif
#endif /* CONFIG_CUSTOM_VIEW_SHOW_BATTERY */
...
int zmk_widget_status_init(struct zmk_widget_status *widget, lv_obj_t *parent) {
    ...
    /* Drei 68x68-Canvases – horizontal arrangiert.
       Hinweis: Bei 160 px Breite überlappen 3x 68 px leicht; 54/108 ist ein guter Offset. */
    lv_obj_t *top = lv_canvas_create(widget->obj);
#if IS_ENABLED(CONFIG_CUSTOM_VIEW_ROTATE_CANVAS)
    /* Alte Portrait-Ausrichtung: behalte deine bisherigen Offsets */
...
#else
    /* Landscape: rechts */
    lv_obj_align(bottom, LV_ALIGN_TOP_LEFT, 108, 0);
#endif
